  ...
  return (
    <GameShell accent={ACCENT}>
      <div
        ...
        style={{
          backgroundImage: "radial-gradient(circle, #E8735A 1px, transparent 1px)",
      ...
      <AnimatePresence mode="wait">
        {phase === "intro" ? (
          ...
          >
            <motion.div
              className="relative mb-7 grid h-28 w-28 place-items-center rounded-full border border-[#E8735A]/30"
              animate={{
                boxShadow: [
                  "0 0 0 0 rgba(232,115,90,0.05)",
                  "0 0 0 18px rgba(232,115,90,0.08)",
                  "0 0 0 0 rgba(232,115,90,0.05)",
                ],
              }}
            ...
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.34em] text-[#E8735A]">
              Un momento per te
            </p>
            <h3 className="font-display text-3xl font-semibold text-white">Che cosa senti?</h3>
            <p className="mt-3 max-w-xs text-sm leading-relaxed text-[#a9a2bc]">
              Lascia che la ruota scelga una parola. Non serve giudicarla: basta ascoltarla.
            </p>
            <motion.button
              onClick={spin}
              className="mt-8 min-h-12 rounded-full bg-[#E8735A] px-7 py-3 text-sm font-semibold text-white shadow-[0_0_28px_rgba(232,115,90,0.28)] cursor-pointer"
              whileHover={{ scale: 1.04, boxShadow: "0 0 38px rgba(232,115,90,0.45)" }}
              whileTap={{ scale: 0.96 }}
            >
              Inizia ad ascoltare
            </motion.button>
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.3em] text-[#E8735A]">
            ...
            <div className="relative h-[250px] w-[250px] sm:h-[280px] sm:w-[280px]">
              <motion.div
                className="absolute inset-1 rounded-full bg-[#E8735A]/20 blur-2xl"
                animate={
                  phase === "complete"
              ...
              >
                ...
                >
                  <circle cx="50" cy="50" r="49" fill="#160f27" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
                  ...
                  <circle cx="50" cy="50" r="12" fill="#160f27" stroke="rgba(255,255,255,0.28)" strokeWidth="1" />
              ...
              >
                <div className="h-0 w-0 border-x-[12px] border-t-[22px] border-x-transparent border-t-white drop-shadow-[0_3px_8px_rgba(255,255,255,0.45)]" />
              </motion.div>
              {phase === "complete" &&
                Array.from({ length: 12 }, (_, index) => (
                  <motion.span
                    key={index}
                    className="absolute left-1/2 top-1/2 h-1.5 w-1.5 rounded-full bg-[#E8735A]"
            ...
            <AnimatePresence mode="wait">
              {phase === "active" ? (
                <motion.p
                  key="listening"
                  className="mt-3 text-sm italic text-[#a9a2bc]"
                  initial={{ opacity: 0 }}
                ...
                selected && (
                  ...
                  >
                    <h3 className="font-display text-4xl font-semibold text-[#ff9b87]">
                      {selected.name}
                    </h3>
                    <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-[#d1cadf]">
                      {selected.prompt}
                    </p>
                  </motion.div>
                )
