...
export default function FinalePage() {
  ...
  return (
    <main className="min-h-dvh flex items-center justify-center bg-[#0f0a1a] px-5 sm:px-6 py-12 safe-inset">
      <div className="max-w-lg mx-auto text-center stagger">
        <div className="mb-6 sm:mb-8">
          <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto rounded-full bg-gradient-to-br from-[#e85a8f] to-[#5ae8c8] flex items-center justify-center animate-pulse-glow">
            <span className="text-xl sm:text-2xl">✦</span>
          </div>
        </div>

        <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4">Complimenti! 🎉</h1>
        <p className="text-[#8b85a0] text-base sm:text-lg mb-6 sm:mb-8">Hai completato tutte e sei le tappe del percorso.</p>

        <div className="p-5 sm:p-8 rounded-2xl bg-gradient-to-br from-[#1a1230] to-[#2d1b69] border border-white/10 mb-6 sm:mb-8">
          <p className="text-xs sm:text-sm text-[#8b85a0] mb-3 sm:mb-4">Le sei parole:</p>
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {STEPS.map(step => (
              <span key={step.id} className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-sm sm:text-lg font-bold tracking-wider" style={{ backgroundColor: `${step.color}20`, color: step.color }}>
                {step.word}
              </span>
            ))}
          </div>
          <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-white/10">
            <p className="text-base sm:text-xl md:text-2xl font-light italic text-[#a09ab5] leading-relaxed">“{FULL_PHRASE}”</p>
          </div>
        </div>

        <div className="p-5 sm:p-8 rounded-2xl bg-[#1a1230] border border-[#e85a8f]/20">
